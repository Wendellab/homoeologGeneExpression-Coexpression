...
for(file in rdatafiles)
{
    ...
    exp<-as.data.frame(multiExpr[[which(shortLabels=="A2D5")]]$data)
    obs<-as.data.frame(multiExpr[[which(shortLabels=="ADs")]]$data)
    
    # Peason's correlation
    exp.r <- cor(exp,method = "pearson",nThread=8)
    obs.r <- cor(obs,method = "pearson",nThread=8)
    ...
    for(net in 1:nrow(netType))
    {
        ...
        for(func in c("kegg","go","fa","ft")){
            ...
            labels <- prep_annot(t(exp), annotation, min = 10, max = 500)
            # get fc
            message(paste0("......",ncol(exp.net$A)," At vs ",ncol(exp.net$D)," Dt genes."))
            exp.fc <- list(A =neighbor_voting(labels, exp.net$A, output = 'AUROC'), D=neighbor_voting(labels, exp.net$D, output = 'AUROC'), interAD=neighbor_voting(labels, exp.net$interAD, output = 'AUROC'))
            obs.fc <- list(A =neighbor_voting(labels, obs.net$A, output = 'AUROC'), D=neighbor_voting(labels, obs.net$D, output = 'AUROC'), interAD=neighbor_voting(labels, obs.net$interAD, output = 'AUROC'))
...
# about 15min x 11 nets x 20 = 330 min, about 6 hrs
