...
---book
rfiles<- grep("R-04",list.files(),value=TRUE)
file<-rfiles[3]
print(file)
load(file)->l;l
...
rdatafiles<-grep("R-02-dataInput",list.files(),value=TRUE)
rdatafiles<-grep("DP",rdatafiles,invert=TRUE,value=TRUE)
rdatafiles
...
for(file in rdatafiles)
{
    ...
    write.table(Rtable, file = paste0("s4.wgcna/parameters.",compr,".txt"),sep="\t")
    save( list=c( grep("Concepts",ls(), value=TRUE)), file = paste0("s4.wgcna/networkTopology.", compr,".RData"))
}
---book
    ...
    pdf(paste0("s4.wgcna/correlatingParameters.",compr,".k.pdf") )
    ...
    dev.off()
    pdf(paste0("s4.wgcna/correlatingParameters.",compr,".sk.pdf") )
