...
################## {getOrigin.sh} ############################
DIR="/work/LAS/jfw-lab/hugj2006/eflen2020/bowtie2hylite/resultsA2"
FLAG="resultsA2_011820"

cd $DIR
module load samtools perl
for j in $( ls $DIR | grep 'ADs.*sorted.bam$' );        do
    echo $j
    echo ${j/ADs/A2}
    echo ${j/ADs/D5}
    echo ${j%%.*}.origin.txt
    ...
    # reads truly from A2
    samtools view ${j/ADs/A2} | awk '{print $1}' >A2.txt
    # reads truly from D5
    samtools view ${j/ADs/D5}| awk '{print $1}' >D5.txt
    # obtain origin and paste to last column of hylite output
    perl /work/LAS/jfw-lab/hugj2006/eflen2020/Ranalysis/labelAD.pl A2.txt D5.txt AD.id.txt | paste ${DIR}_${FLAG}.ADs.${j%%.*}.read.txt - > origin.${j%%.*}.txt
